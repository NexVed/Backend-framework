...
async function updatePackageJson(projectPath: string, config: ProjectConfig) {
    ...
    const dbDependencies: Record<string, string> = {
    ...
    const typesDeps: Record<string, string> = {
        pg: '@types/pg',
    };
...
async function updateBackendConfig(projectPath: string, config: ProjectConfig) {
    ...
    if (config.databases.length > 0) {
        content = content.replace(
            /default: '[^']*'/,
            `default: '${config.databases[0]}'`
        );
    }

    // Update auth setting
    content = content.replace(
        /auth:\s*\{[\s\S]*?enabled:\s*(true|false)/,
        `auth: {\n    enabled: ${config.enableAuth}`
    );

    // Update cache settings
    content = content.replace(
        /cache:\s*\{[\s\S]*?enabled:\s*(true|false)/,
...
function getDbConfigBlock(db: string): string {
...
async function updateEnvExample(projectPath: string, config: ProjectConfig) {
...
`;
    ...
    content = content.replace(
        '# DATABASE PROVIDERS',
        `# DATABASE PROVIDERS${selectedNote}`
    );

    await fs.writeFile(envPath, content);
}

async function cleanupUnusedFiles(projectPath: string, config: ProjectConfig) {
